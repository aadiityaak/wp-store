...
?>
<form method="get" action="" class="wps-card wps-p-4" style="margin-bottom:12px;">
  ...
  <div class="wps-grid wps-grid-cols-2 wps-gap-3">
    <div>
      ...
      <select class="wps-select" name="sort">
    ...
    <div class="wps-grid wps-grid-cols-2 wps-gap-3">
      <div>
        ...
        <input class="wps-input" type="number" min="0" step="1" name="min_price" value="<?php echo esc_attr($current['min_price']); ?>">
      </div>
      <div>
        <label class="wps-label">Harga Maksimum</label>
        <input class="wps-input" type="number" min="0" step="1" name="max_price" value="<?php echo esc_attr($current['max_price']); ?>">
      </div>
    </div>
  </div>
  <div class="wps-mt-3">
    ...
    <div class="wps-flex wps-flex-wrap" style="gap:8px;">
      <?php foreach ($categories as $cat): ?>
        <label class="wps-checkbox-label" style="display:inline-flex; align-items:center; gap:6px;">
          <input type="checkbox" class="wps-checkbox" name="cats[]" value="<?php echo esc_attr($cat['id']); ?>" <?php echo in_array($cat['id'], $current['cats'], true) ? 'checked' : ''; ?>>
  ...
  <div class="wps-mt-3">
    ...
    <div class="wps-flex wps-flex-wrap" style="gap:8px;">
      <label class="wps-checkbox-label" style="display:inline-flex; align-items:center; gap:6px;">
        <input type="checkbox" class="wps-checkbox" name="labels[]" value="best" <?php echo in_array('best', $current['labels'], true) ? 'checked' : ''; ?>>
        <span class="wps-text-sm wps-text-gray-900">Best Seller</span>
      </label>
      <label class="wps-checkbox-label" style="display:inline-flex; align-items:center; gap:6px;">
        <input type="checkbox" class="wps-checkbox" name="labels[]" value="limited" <?php echo in_array('limited', $current['labels'], true) ? 'checked' : ''; ?>>
        <span class="wps-text-sm wps-text-gray-900">Limited</span>
      </label>
      <label class="wps-checkbox-label" style="display:inline-flex; align-items:center; gap:6px;">
        <input type="checkbox" class="wps-checkbox" name="labels[]" value="new" <?php echo in_array('new', $current['labels'], true) ? 'checked' : ''; ?>>
